...
const PartnerFormModal = ({ isOpen, onClose, onSave, partnerToEdit }) => {
    ...
    const [fleetFiles, setFleetFiles] = useState([]);
    ...
    useEffect(() => {
        if (partnerToEdit) {
            ...
            // Initialize file states with nulls to match existing array lengths
            setFleetFiles(new Array(partnerToEdit.fleets?.length || 0).fill(null));
    ...
    const addArrayItem = (arrayName, item) => {
        ...
        if (arrayName === 'fleets') setFleetFiles(prev => [...prev, null]);
    ...
    const removeArrayItem = (index, arrayName) => {
        ...
        if (arrayName === 'fleets') setFleetFiles(prev => prev.filter((_, i) => i !== index));
        if (arrayName === 'sections') setSectionFiles(prev => prev.filter((_, i) => i !== index));
    };

    const handleFileChange = (index, file, fileArraySetter) => {
        const newFiles = [...(fileArraySetter === setFleetFiles ? fleetFiles : sectionFiles)];
        newFiles[index] = file;
    ...
    const handleSubmit = async (e) => {
        ...
        // Append complex array data as JSON strings
        // The backend will need to JSON.parse these fields
        data.append('faqs', JSON.stringify(formData.faqs));

        // For arrays with files, stringify the text data and append files separately
        const fleetsWithoutFiles = formData.fleets.map(f => ({ ...f, image: f.image || '' }));
        const sectionsWithoutFiles = formData.sections.map(s => ({ ...s, image: s.image || '' }));

        data.append('fleets', JSON.stringify(fleetsWithoutFiles));
        data.append('sections', JSON.stringify(sectionsWithoutFiles));

        // Append the actual files
        fleetFiles.forEach(file => { if (file) data.append('fleetImages', file); });
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start z-50 overflow-y-auto">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl my-8 p-6">
                <h2 className="text-2xl font-bold mb-6 text-gray-800">{partnerToEdit ? 'Edit' : 'Create'} Driver Partner</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <fieldset className="border p-4 rounded-md">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <input name="city" value={formData.city} onChange={handleInputChange} placeholder="City (e.g., Delhi)" className="p-2 border rounded" required />
                    ...
                    <fieldset className="border p-4 rounded-md space-y-4">
                        <legend className="text-lg font-semibold px-2 text-gray-700 flex items-center gap-2"><Car size={20} /> Fleets</legend>
                        {formData.fleets.map((fleet, index) => (
                            <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-3 p-3 bg-gray-50 rounded-md relative">
                                <input value={fleet.vehicleType} onChange={(e) => handleArrayChange(index, 'vehicleType', e.target.value, 'fleets')} placeholder="Vehicle Type (e.g., Sedan)" className="p-2 border rounded" />
                                <input type="number" value={fleet.capacity} onChange={(e) => handleArrayChange(index, 'capacity', e.target.value, 'fleets')} placeholder="Capacity" className="p-2 border rounded" />
                                ...
                                <input type="file" accept="image/*" onChange={(e) => handleFileChange(index, e.target.files[0], setFleetFiles)} className="p-1 border rounded text-sm" />
                        ...
                        <button type="button" onClick={() => addArrayItem('fleets', { vehicleType: '', description: '', capacity: '' })} className="text-sm px-3 py-1 bg-blue-100 text-blue-800 rounded hover:bg-blue-200">
                            Add Vehicle
                        </button>
                    </fieldset>
...
export default function ManageDriverPartners() {
    ...
    const fetchPartners = async () => {
        ...
        try {
            ...
            setError('Failed to fetch driver partners.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this driver partner entry?')) {
            try {
                await deletePartner(id);
                fetchPartners();
            } catch (err) {
                alert('Failed to delete driver partner.');
            }
        }
    };
    ...
    const handleUpload = async () => {
        ...
        formData.append('driverPartnersFile', selectedFile);
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-semibold">Manage Driver Partners</h1>
            ...
            <h2 className="text-xl font-semibold mt-8 mb-4">Existing Driver Partners</h2>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {partners.map((partner) => (
                                <tr key={partner._id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center">{partner.fleets?.length || 0}</td>
