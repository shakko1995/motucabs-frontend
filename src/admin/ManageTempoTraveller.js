...
import { Plus, Edit, Trash2, Loader, AlertCircle, Upload, File, CheckCircle, AlertTriangle, Package, HelpCircle, FileText, Bus } from 'lucide-react';
...
// --- API Functions ---
const getAllTravellers = () => axios.get(API_URL, getAuthHeaders());
const createTraveller = (data) => axios.post(API_URL, data, getAuthHeaders());
const updateTraveller = (id, data) => axios.put(`${API_URL}/${id}`, data, getAuthHeaders());
const deleteTraveller = (id) => axios.delete(`${API_URL}/${id}`, getAuthHeaders());
const uploadTravellersSheet = (formData) => axios.post(`${API_URL}/upload-sheet`, formData, {
     headers: { ...getAuthHeaders().headers, 'Content-Type': 'multipart/form-data' }
});


// --- Form Modal Component ---
const TravellerFormModal = ({ isOpen, onClose, onSave, travellerToEdit }) => {
    ...
    useEffect(() => {
        if (travellerToEdit) {
            setFormData({ ...initialFormState, ...travellerToEdit });
            setSectionFiles(new Array(travellerToEdit.sections?.length || 0).fill(null));
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(formData).forEach(key => {
            if (!['packages', 'faqs', 'sections', '_id', 'createdAt', 'updatedAt', '__v', 'slug'].includes(key)) {
        ...
        try {
            if (travellerToEdit) {
                await updateTraveller(travellerToEdit._id, data);
            } else {
                await createTraveller(data);
            }
            onSave();
            onClose();
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start z-50 overflow-y-auto">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl my-8 p-6">
                <h2 className="text-2xl font-bold mb-6 text-gray-800">{travellerToEdit ? 'Edit' : 'Create'} Tempo Traveller Page</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <fieldset className="border p-4 rounded-md space-y-4">
                        <legend className="text-lg font-semibold px-2 text-gray-700 flex items-center gap-2"><Package size={20} /> Packages</legend>
                        {formData.packages.map((pkg, index) => (
                            <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-3 p-3 bg-gray-50 rounded-md relative">
                                <input value={pkg.packageName} onChange={(e) => handleArrayChange(index, 'packageName', e.target.value, 'packages')} placeholder="Package Name (e.g., 12 Seater)" className="p-2 border rounded" />
                                <input type="number" value={pkg.capacity} onChange={(e) => handleArrayChange(index, 'capacity', e.target.value, 'packages')} placeholder="Capacity (e.g., 12)" className="p-2 border rounded" />
                                <input value={pkg.description} onChange={(e) => handleArrayChange(index, 'description', e.target.value, 'packages')} placeholder="Description" className="p-2 border rounded" />
                                <button type="button" onClick={() => removeArrayItem(index, 'packages')} className="absolute top-1 right-1 text-red-500 hover:text-red-700"><Trash2 size={16} /></button>
                            </div>
                        ))}
                        <button type="button" onClick={() => addArrayItem('packages', { packageName: '', capacity: '', description: '' })} className="text-sm px-3 py-1 bg-blue-100 text-blue-800 rounded hover:bg-blue-200">
                            Add Package
                        </button>
                    </fieldset>
...
export default function ManageTempoTravellers() {
    ...
    const fetchTravellers = async () => {
        ...
        setError('');
        try {
            const response = await getAllTravellers();
            setTravellers(response.data);
        } catch (err) {
            setError('Failed to fetch Tempo Traveller data.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this Tempo Traveller entry?')) {
            try {
    ...
    const handleUpload = async () => {
        ...
        formData.append('tempoTravellersFile', selectedFile);

        try {
            const response = await uploadTravellersSheet(formData);
    ...
    return (
        <div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200 my-8">
                <h2 className="text-xl font-semibold mb-4 text-gray-800">Bulk Upload from Excel</h2>
            ...
            <h2 className="text-xl font-semibold mt-8 mb-4">Existing Tempo Traveller Pages</h2>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {travellers.map((traveller) => (
                                <tr key={traveller._id}>
                                    <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">{traveller.city}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-gray-600">{traveller.title}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-center">{traveller.packages?.length || 0}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-center">{traveller.faqs?.length || 0}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button onClick={() => handleEdit(traveller)} className="text-indigo-600 hover:text-indigo-900 mr-4"><Edit size={18} /></button>
                                        <button onClick={() => handleDelete(traveller._id)} className="text-red-600 hover:text-red-900"><Trash2 size={18} /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            {isModalOpen && <TravellerFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} travellerToEdit={currentTraveller} />}
        </div>
    );
}