...
//     const res = await axios.post('https://api.cloudinary.com/v1_1/b7klcilWxQjgR4sk1H7a/image/upload', data); // replace with your cloud name
...
//   //     const sectionsWithImages = await Promise.all(formData.sections.map(async (section, index) => {
//   //       if (sectionFiles[index]) {
//   //         const url = await uploadImageToCloudinary(sectionFiles[index]);
//   //         return { ...section, image: url };
//   //       }
//   //       return section;
//   //     }));

//   //     const payload = { ...formData, sections: sectionsWithImages };
...
//       formPayload.append('atAGlance', formData.atAGlance || '');
//       formPayload.append('extraInfo', formData.extraInfo || '');
//       formPayload.append('descriptions', JSON.stringify(formData.descriptions));
//       formPayload.append('fares', JSON.stringify(formData.fares));
//       formPayload.append('whyBook', JSON.stringify(formData.whyBook));
//       formPayload.append('faqs', JSON.stringify(formData.faqs));

//       // Sections: include old image URLs and new files
//       const sectionsToSend = formData.sections.map((section, index) => {
//         return {
//           ...section,
//           image: sectionFiles[index] ? undefined : section.image || ''
//         };
//       });
//       formPayload.append('sections', JSON.stringify(sectionsToSend));

//       // Append new section files
//       Object.entries(sectionFiles).forEach(([index, file]) => {
//         formPayload.append('sectionFiles', file);
//       });
...
const ManageOutstationsSiteMap = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Append sections JSON with placeholders for new images
      const sectionsToSend = formData.sections.map((section, index) => ({
        ...section,
        image: sectionFiles[index] ? '' : section.image || ''
      }));
      formPayload.append('sections', JSON.stringify(sectionsToSend));

      // Append each file with unique key
      // Append all new section files under the same key
Object.entries(sectionFiles).forEach(([index, file]) => {
  formPayload.append('sectionImages', file); // same key for all files
});
      ...
      await axios[method](url, formPayload, {
        headers: { Authorization: `Bearer ${adminToken}` } // Axios handles Content-Type automatically
      });
  ...
  const handleDelete = async (slug) => {
    if (!window.confirm('Are you sure?')) return;
    try {
      await axios.delete(`http://localhost:5000/api/outstations/${slug}`, {
        headers: { Authorization: `Bearer ${adminToken}` }
      });
      alert('Outstation deleted successfully!');
      fetchOutstations();
    } catch (err) {
      setError('Failed to delete outstation.');
    }
  };
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-full">
      ...
      {/* Form */}
      <div className="bg-white p-8 rounded-lg shadow-md mb-8">
        <h2 className="text-2xl font-semibold mb-6">{editMode ? `Editing: ${formData.name}` : 'Add New Outstation'}</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Name & Slug */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input name="name" value={formData.name} onChange={handleInputChange} placeholder="Location Name" className="p-3 border rounded-md" required />
          ...
          {/* Fares */}
          <fieldset className="border p-4 rounded-md">
            <legend className="px-2 font-semibold">Fares</legend>
            {formData.fares.map((fare, index) => (
              <div key={index} className="grid grid-cols-3 gap-2 items-center mb-2">
                <input name="route" value={fare.route} onChange={(e) => handleNestedChange('fares', index, e)} placeholder="Route" className="p-2 border rounded-md" />
                <input name="price" type="number" value={fare.price} onChange={(e) => handleNestedChange('fares', index, e)} placeholder="Price" className="p-2 border rounded-md" />
          ...
          {/* Why Book */}
          <textarea name="whyBook" value={formData.whyBook.join('\n')} onChange={handleInputChange} placeholder="Why Book" rows="4" className="w-full p-3 border rounded-md" />
