...
export async function getAllTempoTravellerRoutes() {
...
export async function getAllTempoTravellersRoutes() {
  try {
    const res = await fetch(API_URL); // or a specific endpoint if needed
    if (!res.ok) throw new Error("Failed to fetch tempo travellers routes");
    const data = await res.json();
    return data || [];
  } catch (err) {
    console.error("Error fetching tempo travellers routes:", err);
    return [];
  }
}
