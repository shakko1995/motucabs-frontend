...
import { Star, ArrowLeft, Car, Users, ShieldCheck, CalendarDays, Filter, X, TrendingUp, TrendingDown, Gauge, Clock, Moon, Droplets, Snowflake } from 'lucide-react';
    ...
    return (
      <div className={`relative bg-white rounded-xl shadow-lg p-5 flex flex-col sm:flex-row items-center gap-5 border-2 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 ${isRecommended ? 'border-blue-500' : 'border-transparent'}`}>
        ...
        <div className="w-full sm:w-1/4 text-center sm:text-right border-t sm:border-t-0 sm:border-l-2 border-dashed pl-0 sm:pl-5 pt-4 sm:pt-0">
          ...
          <button onClick={onSelect} className="w-full  mt-2 sm:w-auto bg-orange-500 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-orange-600 transition-all duration-300 transform hover:scale-105">SELECT</button>
        </div>
      </div>
    );
};
...
const FilterPanel = ({ sortBy, setSortBy, filters, handleFilterChange, onClose }) => (
    <div className="bg-white p-5 rounded-xl shadow-lg h-full overflow-y-auto">
       ...
       <div className="mt-6 border-t pt-4">
            ...
            <div className="flex flex-col space-y-1">
                <button onClick={() => handleFilterChange('fuelType', 'all')} className={`text-left text-sm p-3 rounded-lg font-medium ${filters.fuelType === 'all' ? 'bg-blue-600 text-white shadow' : 'text-slate-700 hover:bg-slate-100'}`}>Any</button>
                <button onClick={() => handleFilterChange('fuelType', 'Petrol')} className={`text-left text-sm p-3 rounded-lg font-medium ${filters.fuelType === 'Petrol' ? 'bg-blue-600 text-white shadow' : 'text-slate-700 hover:bg-slate-100'}`}>Petrol</button>
                <button onClick={() => handleFilterChange('fuelType', 'Diesel')} className={`text-left text-sm p-3 rounded-lg font-medium ${filters.fuelType === 'Diesel' ? 'bg-blue-600 text-white shadow' : 'text-slate-700 hover:bg-slate-100'}`}>Diesel</button>
            </div>
       </div>
    </div>
);
...
const OutstationRoundTripCabs = () => {
  ...
  const handleSelectCab = (selectedPackage) => {
    const tripDetails = {
        from, to, pickupDate, dropDate, 
        pickupTime: time, dropTime: returnTime, 
        tripDays, tripType: 'Outstation Round Trip'
    };
    navigate('/review-booking', { 
        state: { packageDetails: selectedPackage, tripDetails: tripDetails } 
    });
  };
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <main className="lg:col-span-3">
            ...
            {error && <div className="text-center p-12 bg-white rounded-xl shadow-lg"><p className="text-red-600 font-bold">{error}</p></div>}
            {!error && packages.length > 0 && (
              <div className="space-y-6">
                {displayedPackages.map(pkg => (<RoundTripCabCard key={pkg._id} packageDetails={pkg} isRecommended={pkg.isRecommended} onSelect={() => handleSelectCab(pkg)} />))}
              </div>
            )}
