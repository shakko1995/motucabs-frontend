...
import { useNavigate, Link } from 'react-router-dom';
import { AuthContext } from '../context/AuthContext';
import { navigationLinks } from '../data/navigationData'; // इसमें icons already defined होने चाहिए
import { User, LogOut, ChevronDown, Menu, X } from 'lucide-react';
            ...
            {navigationLinks.map(nav => (
                <div key={nav.title} className="border-b pb-2">
                    ...
                    >
                        ...
                        <ChevronDown 
                            size={20} 
                            className={`transition-transform ${openAccordion === nav.title ? 'rotate-180' : ''}`} 
                        />
                    </div>
                    <div className={`overflow-hidden transition-all duration-300 ${openAccordion === nav.title ? 'max-h-96 pt-2' : 'max-h-0'}`}>
                        <ul className="pl-4 space-y-1">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="relative">
                        ...
                        {showCountryDropdown && (
                            <div className="absolute top-full mt-1 right-0 bg-white border rounded-lg shadow-lg z-50 w-40">
                                <button 
                                    onClick={() => { setSelectedCountry("IN"); setShowCountryDropdown(false); }} 
                                    className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50"
                                >
                                    India (IN)
                                </button>
                                <button 
                                    onClick={() => { setSelectedCountry("USA"); setShowCountryDropdown(false); }} 
                                    className="w-full px-4 py-2 text-left text-sm hover:bg-gray-50"
                                >
                                    USA
                                </button>
                    ...
                    {user ? (
                        <div className="relative group">
                            ...
                            <div className="absolute top-full right-0 mt-1 w-48 bg-white rounded-md shadow-lg border z-20 hidden group-hover:block">
                                <button 
                                    onClick={logout} 
                                    className="w-full px-4 py-2.5 text-left text-red-600 hover:bg-red-50 flex items-center text-sm font-medium"
                                >
                                    <LogOut className="w-4 h-4 mr-2" />
                                    Logout
                                </button>
